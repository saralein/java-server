...
public class PartialContentController implements Controller {
    ...
    public PartialContentController(ServerPaths serverPaths, FilesInfo files, FileIO fileIO) {
    ...
    private Response determineRangeResponse(Request request) throws IOException {
        Path file = serverPaths.createAbsolutePath(request.getUri());
        int fileLength = files.getFileLength(file);
    ...
    private Response buildValidRangeResponse(Request request, int fileLength, int start, int end) throws IOException {
        ...
        return new Response.Builder()
                .status(206)
                .addHeader("Content-Type", files.determineMimeType(request.getUri()))
